...
:: make clean
:: "%BUILDERDIR%\devenv.exe" "%PROJDIR%\CUnit.sln" /Clean

:: More info on commands used below - refer http://www.klocwork.com/products/documentation/current/Command_Reference"

:: Uncomment and modify appropriate command for build spec creation from below reference sample commands
:: -------------------

:: -------------------
::echo "Creating build specification - %PROJNAME%"
:: "%KWROOT%\bin\kwinject.exe" -o "%KWTABLES%\%PROJNAME%.out" "%BUILDERDIR%\msbuild.exe" "%PROJDIR%\Plugin.sln" /t:build /property:Configuration="Debug"
:: -------------------
 echo "Creating build specification - %PROJNAME%"
...
:: -------------------
:: echo "Creating build specification - %PROJNAME%"
:: "%KWROOT%\bin\kwant.exe" -o "%KWTABLES%\%PROJNAME%.out"
:: -------------------
:: echo "Creating build specification - %PROJNAME%"
:: "%KWROOT%\bin\kwdspparser.exe" "%PROJDIR%\CUnit.dsp" --list-configs
:: "%KWROOT%\bin\kwdspparser.exe" -o "%KWTABLES%\%PROJNAME%.out" "%PROJDIR%\CUnit.dsp" --config "Debug|Win32"
:: -------------------
...
:: echo "Synchronizing issue status update between multiple related projects / variants"
:: kwxsync %KWHOSTURL%/%PROJNAME1% %KWHOSTURL%/%PROJNAME2%
